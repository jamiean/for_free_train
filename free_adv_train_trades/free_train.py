...
def train(tf_seed, np_seed, train_steps, out_steps, summary_steps, checkpoint_steps, step_size_schedule,
    ...
    if dataset == 'cifar10':
      ...
      raw_data = MNIST_input.MNISTData()
    else:
    ...
    with tf.Session() as sess:
        ...
        if dataset == 'cifar10':
          ...
          data = MNIST_input.AugmentedMNISTData(raw_data, sess, model)
        else:
        ...
        for ii in range(train_steps):
            if ii % replay_m == 0:
                ...
                nat_dict = {model.x_input: x_batch, model.x_input_nat: x_batch, model.y_input: y_batch}

            x_eval_batch, y_eval_batch = data.eval_data.get_next_batch(train_batch_size, multiple_passes=True)
            eval_dict = {model.x_input: x_eval_batch, model.x_input_nat: x_eval_batch, model.y_input: y_eval_batch}
