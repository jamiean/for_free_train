...
class MNISTData(object):
    ...
    def __init__(self):
        ...
        train_filename = 'train_MNIST'
        eval_filename = 'test_MNIST'#'test_batch'
        metadata_filename = 'meta_MNIST'#'batches.meta'

        train_images = np.zeros((55000, 32, 32, 3), dtype='uint8')
        train_labels = np.zeros(55000, dtype='int32')
        raw_data_MNIST = input_data.read_data_sets('MNIST_data', one_hot=False)
        ...
        train_images = raw_data_MNIST.train.images
        train_labels = raw_data_MNIST.train.labels
        eval_images  = raw_data_MNIST.test.images
        eval_labels = raw_data_MNIST.test.labels
...
class AugmentedMNISTData(object):
    ...
    """
    def __init__(self, raw_MNISTdata, sess, model):
        assert isinstance(raw_MNISTdata, MNISTData)
        self.image_size = 32
        ...
        self.train_data = AugmentedDataSubset(raw_MNISTdata.train_data, sess,
                                             self.x_input_placeholder,
                                              self.augmented)
        self.eval_data = AugmentedDataSubset(raw_MNISTdata.eval_data, sess,
                                             self.x_input_placeholder,
                                             self.augmented)
        self.label_names = raw_MNISTdata.label_names
